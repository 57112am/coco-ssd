...
function App() {
  ...
  const [resourceUsage, setResourceUsage] = useState({ memory: {}, cpu: [] });
  ...
  useEffect(() => {
    ...
    socket.on("predictions", (predictions) => {
      ...
      predictions.forEach((prediction) => {
        ...
        context.font = "16px Arial"; // Set font for text
        context.fillText(
          text,
          prediction.bbox[0],
  ...
  return (
    <div className="w-full h-full p-5  bg-gray-800">
      ...
      <div className="-mt-20 text-white overflow-y-auto max-h-96">
        <h4>Memory Usage:</h4>
        <p>Total: {resourceUsage.memory.total}</p>
        <p>Used: {resourceUsage.memory.used}</p>
